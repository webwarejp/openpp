$(document).ready(function(){
  ...
  $(window).bind('bottom', function() {
    ...
    if (!obj.data('loading')) {
      ...
      // 読み込み中の画像を表示
      $('#loadimg').html('<img src="/bundles/acmetimeline/image/loading.gif" />');
      setTimeout(function() {
        // ここにajax()やgetJSON()など
        // 実際に次のコンテンツを取得する処理を記述します
        $.ajax({
            type:'GET',
            url: Routing.generate('acme_timeline_get') + '/' + $('#timeline').attr('data-id') + '/' + parseInt($('#timeline').attr('data-page')+1),
            dataType: 'json',
